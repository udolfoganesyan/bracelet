...
final class ConversationsViewController: UIViewController {
    ...
    @objc private func handleAddChannel() {
        let alert = UIAlertController(title: "Новый контакт", message: nil, preferredStyle: .alert)
        
        alert.addTextField { (textField: UITextField) in
            textField.placeholder = "Имя контакта..."
            textField.addTarget(self, action: #selector(self.textChanged(_:)), for: .editingChanged)
        }
        
        let createAction = UIAlertAction(title: "Добавить", style: .default, handler: { _ in
